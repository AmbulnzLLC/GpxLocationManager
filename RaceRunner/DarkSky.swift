...
open class DarkSky {
    ...
    fileprivate func call(_ method: String, callback: @escaping (Result) -> ()) {
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: queue) { (response: URLResponse?, data: Data?, error: Error?) -> Void in
            ...
            currentQueue?.addOperation {
                ...
                if error != nil {
                    result = Result.Error(response, error as NSError?)
                }
                callback(result)
            }
        }
