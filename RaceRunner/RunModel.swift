                    ...
                    longitude: initialLocation.coordinate.longitude) ) { result in
                        ...
                        case .success(_, let dictionary):
                            if let dict = dictionary,
                                let currently = dict["currently"] as? NSDictionary,
                                let temperature = currently["apparentTemperature"] as? Float,
                                let weather = currently["summary"] as? String
                                {
                                self.temperature = Stringifier.convertFahrenheitToCelsius(temperature)
                                self.weather = weather                                //let synth = AVSpeechSynthesizer()
